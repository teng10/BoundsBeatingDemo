...
\begin{definition}[Shallow circuit]
  \label{def:shallow-circuit}
  \lean{ShallowCircuit}
  \uses{def:shallow-layer}
  An $L$-layer circuit $U = U_L \cdots U_1$ is \emph{shallow} if every layer
  $U_j$ satisfies the shallow layer assumption (Definition~\ref{def:shallow-layer}).
  In particular, for any $P$ of weight $|P| = k$, the fully Heisenberg-evolved
  operator $U^\dagger P\, U$ expands into at most $n^{\mathcal{O}(kL)}$ Pauli terms.
\end{definition}
...
% The proof intermediate steps are organized into three lemmas but not currently formalized
...
\begin{theorem}[Mean squared truncation error — depth-independent bound]
  ...
  \lean{mseBound}
  \uses{
    % lem:telescoping, lem:cross-terms, lem:per-layer,
